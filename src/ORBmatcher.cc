...
//******************************** new added searching matches method *********************************
int ORBmatcher::SearchByQuad(Frame& CurrentFrame, const Frame& LastFrame, vector<int>& TemperalMatch)
{

    int nmatches = 0, count = 0, nmatch_radius = 150;  // 200 240 25 50 75 100 125 150
    float u_min1, u_max1, v_min1, v_max1, u_min2, u_max2;
    vector<int> vCandidate;
    ...
    for (int i = 0; i < LastFrame.N; i++)
    {
        ...
        if (pMP)
        {
            ...
            if (!LastFrame.mvbOutlier[i])
            {
                ...
                // float nmatch_radius = th*CurrentFrame.mvScaleFactors[nLastOctave];

                count = count + 1;

                u_min1 = LastFrame.mvKeys[i].pt.x - nmatch_radius;
                if (u_min1 < 0)
                    u_min1 = 0;
                u_max1 = LastFrame.mvKeys[i].pt.x + nmatch_radius;
                if (u_max1 > CurrentFrame.mnMaxX)
                    u_max1 = CurrentFrame.mnMaxX;
                v_min1 = LastFrame.mvKeys[i].pt.y - nmatch_radius;
                if (v_min1 < 0)
                    v_min1 = 0;
                v_max1 = LastFrame.mvKeys[i].pt.y + nmatch_radius;
                if (v_max1 > CurrentFrame.mnMaxY)
                    v_max1 = CurrentFrame.mnMaxY;
                u_min2 = LastFrame.mvuRight[i] - nmatch_radius;
                if (u_min2 < 0)
                    u_min2 = 0;
                u_max2 = LastFrame.mvuRight[i] + nmatch_radius;
                if (u_max2 > CurrentFrame.mnMaxX)
                    u_max2 = CurrentFrame.mnMaxX;
                ...
                for (int j = 0; j < CurrentFrame.N; j++)
                {
                    ...
                    if (nLastOctave + 1 >= 0)
                        if (CurrentFrame.mvKeys[i].octave > nLastOctave + 1)
                            continue;
                    if (CurrentFrame.mvKeys[j].pt.x > u_min1 && CurrentFrame.mvKeys[j].pt.x < u_max1 &&
                        CurrentFrame.mvKeys[j].pt.y > v_min1 && CurrentFrame.mvKeys[j].pt.y < v_max1)
                        if (CurrentFrame.mvuRight[j] > u_min2 && CurrentFrame.mvuRight[j] < u_max2)
                ...
                int bestDist1 = 1000, bestDist2 = 1000;
                ...
                for (int j = 0; j < vCandidate.size(); j++)
                {
                    ...
                    // const double dist1 = cv::norm(LastFrame.mSift.row(i),CurrentFrame.mSift.row(vCandidate[j]),cv::NORM_L2);
                    // const double dist2 = cv::norm(LastFrame.mSiftRight.row(LastFrame.vDescIndex[i]),CurrentFrame.mSiftRight.row(CurrentFrame.vDescIndex[vCandidate[j]]),cv::NORM_L2);
                    if (dist1 < bestDist1)
                    {
                        bestDist1 = dist1;
                ...
                // cout << "best distance: " << bestDist1 << " " << bestDist2 << endl;

                if (bestIdx2 == bestIdx1 && bestIdx1 != -1 && bestDist1 <= 100 && bestDist2 <= 100)
                {  //  && bestDist1<=TH_HIGH && bestDist2<=TH_HIGH  200 for sift

                    CurrentFrame.mvpMapPoints[bestIdx2] = pMP;
                    TemperalMatch[bestIdx2] = i;
                    nmatches++;
...
int ORBmatcher::SearchByQuadKeyFrame(KeyFrame* pKF, Frame& F, vector<MapPoint*> vpMapPointsKF)
{

    int nmatches = 0, nmatch_radius = 240, count = 0;
    float u_min1, u_max1, v_min1, v_max1, u_min2, u_max2;
    vector<int> vCandidate;
    ...
    for (int i = 0; i < pKF->N; i++)
    {
        ...
        if (pMP)
        {
            ...
            u_min1 = pKF->mvKeys[i].pt.x - nmatch_radius;
            u_max1 = pKF->mvKeys[i].pt.x + nmatch_radius;
            v_min1 = pKF->mvKeys[i].pt.y - nmatch_radius;
            v_max1 = pKF->mvKeys[i].pt.y + nmatch_radius;
            u_min2 = pKF->mvuRight[i] - nmatch_radius;
            u_max2 = pKF->mvuRight[i] + nmatch_radius;

            for (int j = 0; j < F.N; j++)
            {
                if (F.vDescIndex[j] == -1)
                    continue;
                if (F.mvKeys[j].pt.x > u_min1 && F.mvKeys[j].pt.x < u_max1 && F.mvKeys[j].pt.y > v_min1 &&
                    F.mvKeys[j].pt.y < v_max1)
                    if (F.mvuRight[j] > u_min2 && F.mvuRight[j] < u_max2)
                        vCandidate.push_back(j);
            }
    ...
    if (mbCheckOrientation)
    {
        ...
        for (int i = 0; i < HISTO_LENGTH; i++)
        {
            if (i != ind1 && i != ind2 && i != ind3)
            {
                for (size_t j = 0, jend = rotHist[i].size(); j < jend; j++)
                {
                    F.mvpMapPoints[rotHist[i][j]] = static_cast<MapPoint*>(NULL);
                    nmatches--;
                }
            }
        }
