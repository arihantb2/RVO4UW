...
cv::Mat Tracking::GrabImageStereo(const cv::Mat& imRectLeft, const cv::Mat& imRectRight, const double& timestamp)
{
    ...
    // **********************************************************
    // suace enhancement (image enhancement)
    // int aa = 42, bb = 122;  // 42 122
    // cv::Mat mImGray2, imGrayRight2;
    // performSUACE(mImGray, mImGray2, aa, (bb + 1) / 8.0);
    // performSUACE(imGrayRight, imGrayRight2, aa, (bb + 1) / 8.0);
    // **********************************************************
    ...
    // **********************************************************
    // equalize the histogram of the images (image enhancement)
    cv::Ptr<cv::CLAHE> clahe = cv::createCLAHE();
    clahe->setClipLimit(4.2);  // 4.2
    clahe->apply(mImGray, mImGray);
    clahe->apply(imGrayRight, imGrayRight);
    // **********************************************************
    ...
    // cv::drawKeypoints(mImGray, mCurrentFrame.mvKeys, mImGray, cv::Scalar(0,0,255), 1); // Scalar::all(-1)
    // string savefile = "/Users/steed/work/code/ORB_SLAM2/clahe.png";
    // cv::imwrite(savefile,mImGray);
    // cv::imshow("CLAHE", mImGray);
    // cv::waitKey(0);
    ...
    // // ************** display valid features ***************
    std::vector<cv::KeyPoint> KeyPointsL;
    int count = 0;
    for (int iL = 0; iL < mCurrentFrame.N; iL++)
    {
        if (mCurrentFrame.mvpMapPoints[iL])
        {
            KeyPointsL.push_back(mCurrentFrame.mvKeys[iL]);
            count = count + 1;
        }
    }
    // cout << "tracking features numeber: " << count <<  endl;
    cv::drawKeypoints(mImGray, KeyPointsL, mImGray, cv::Scalar(0, 0, 255),
                      cv::DrawMatchesFlags::DEFAULT);  // Scalar::all(-1)
    cv::imshow("Valid Features After Tracking", mImGray);
    if (F_id < 53)
        cv::waitKey(1);
    else
        cv::waitKey(0);
...
void Tracking::Track()
{
    ...
    else
    {
        ...
        mvKeysLastFrame = mLastFrame.mvKeys;  // new added (1st Dec)
...
bool Tracking::TrackReferenceKeyFrame()
{
    ...
    bFrame2Frame = false;
    ORBmatcher matcher(0.9, true);  // 0.7  0.9
...
void Tracking::UpdateLastFrame()
{
    ...
    for (size_t j = 0; j < vDepthIdx.size(); j++)
    {
        ...
        if (vDepthIdx[j].first > mThDepth && nPoints > 800)
        {  // 100 200
            break;
        }
    }
...
bool Tracking::TrackWithMotionModel()
{
    ...
    T_counter = T_counter + time_counter;
    cout << T_counter / F_id << " ";

    // cout << "initial matches: " << nmatches << endl;
    // cout << nmatches << endl;
    M_counter = M_counter + nmatches;
    cout << M_counter / F_id << " " << F_id << endl;
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    bool bNeedToInsertClose = (nTrackedClose < 100) && (nNonTrackedClose > 30);  // 100/250 70/50

    // Thresholds
    float thRefRatio = 0.95f;  // 0.75f  0.90f
    if (nKFs < 2)
        thRefRatio = 0.4f;
    ...
    const bool c2 =
        ((mnMatchesInliers < nRefMatches * thRefRatio || bNeedToInsertClose) && mnMatchesInliers > 10);  // 15
    ...
    if (mSensor != System::MONOCULAR)
    {
        ...
        if (!vDepthIdx.empty())
        {
            ...
            for (size_t j = 0; j < vDepthIdx.size(); j++)
            {
                ...
                if (vDepthIdx[j].first > mThDepth && nPoints > 800)  //  100  200
                    break;
            }
